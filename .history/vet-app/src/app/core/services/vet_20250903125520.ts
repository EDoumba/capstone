...
export class VetService {
  ...
  getVetById(id: number): Observable<Vet> {
    return this.http.get<Vet>(`${this.apiUrl}/api/vets/${id}`);
  }

  getFeaturedVets(): Observable<Vet[]> {
    return this.http.get<Vet[]>(`${this.apiUrl}/api/vets/featured`);
  }

  getVetSpecialties(): Observable<string[]> {
    return this.http.get<string[]>(`${this.apiUrl}/api/vets/specialties`;
  }
}