...
export class UserService {
  ...
  addPet(petData: any): Observable<Pet> {
    return this.http.post<Pet>(`${this.apiUrl}/api/user/pets`, petData);
  }

  updatePet(petId: number, petData: any): Observable<Pet> {
    return this.http.put<Pet>(`${this.apiUrl}/api/user/pets/${petId}`, petData);
  }

  deletePet(petId: number): Observable<any> {
    return this.http.delete(`${this.apiUrl}/api/user/pets/${petId}`);
  }
}