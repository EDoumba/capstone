...
export class UserService {
  ...
  getProfile(): Observable<UserProfile> {
    return this.http.get<UserProfile>('/api/user/profile1);
  }

  updateProfile(profileData: any): Observable<UserProfile> {
    return this.http.put<UserProfile>('/api/user/profile', profileData);
  }

  addPet(petData: any): Observable<Pet> {
    return this.http.post<Pet>('/api/user/pets', petData);
  }
